...
print(f' average monthly change:  {round(avg_change,2)}')
...
with open('My_Analysis.txt', 'w') as f:
    ...
    f.write('     Below is my analysis:\n')
    f.write('------------------------------\n')
    f.write(f' Da data set be having {month_counter} rows yall \n')
    f.write(f' Da total be:  {total_profit}\n')
    f.write(f' Dis be da biggest gain: {h_date} {h_change}\n')
    f.write(f' Dis be da biggest loss: {l_date} {l_change}\n')
    f.write(f' Dis be da average monthly change:  {round(avg_change,2)}\n')
